...
void MenuWidget::saga() {
    ...
    connect(m_sagaPlayer, & QMediaPlayer::mediaStatusChanged, [this](QMediaPlayer::MediaStatus status) {
        if (status == QMediaPlayer::EndOfMedia) {
            ...
            // Create a QPushButton for the cup
            QPushButton *cupButton = new QPushButton(this);

            // Create a QIcon for the cup image
            QIcon cupIcon(":/userInterface/sprites/userInterface/cup.png");

            // Get the current size of the QIcon and set its size to half
            QSize iconSize = cupIcon.actualSize(QSize(100,100)); // Get the current size
            iconSize.setWidth(iconSize.width() / 2); // Set the width to half
            iconSize.setHeight(iconSize.height() / 2); // Set the height to half
            cupButton->setIcon(cupIcon);
            cupButton->setIconSize(iconSize);

            // Position the cup button on the right-hand side of the window, a bit higher up
            int cupButtonX = this->width() - cupButton->width() - 10;  // 10 is the offset from the right edge
            int cupButtonY = this->height() / 4;  // Adjust this as needed to position higher up
            cupButton->move(cupButtonX, cupButtonY);

            // Remove the background of the QPushButton
            cupButton->setStyleSheet("background:transparent;");

            // Connect the clicked signal of the button to the showLeaderboard slot
            connect(cupButton, &QPushButton::clicked, this, &MenuWidget::showLeaderboard);

            cupButton->show();

            // Create a QPushButton for the player
            QPushButton *playerButton = new QPushButton(this);

            // Create a QIcon for the player image
            QIcon playerIcon(":/userInterface/sprites/userInterface/player.png");

            // Get the current size of the QIcon and set its size to half
            QSize playerIconSize = playerIcon.actualSize(QSize(100,100)); // Get the current size
            playerIconSize.setWidth(playerIconSize.width() / 2); // Set the width to half
            playerIconSize.setHeight(playerIconSize.height() / 2); // Set the height to half
            playerButton->setIcon(playerIcon);
            playerButton->setIconSize(playerIconSize);

            // Position the player button on the left-hand side of the window, at the same y-position as the cup
            int playerButtonX = 40; // 10 is the offset from the left edge
            int playerButtonY = cupButtonY; // Use the same y-position as the cup button
            playerButton->move(playerButtonX, playerButtonY);

            // Remove the background of the QPushButton
            playerButton->setStyleSheet("background:transparent;");

            // Connect the clicked signal of the button to the relevant slot
            connect(playerButton, &QPushButton::clicked, this, &MenuWidget::showPlayerOptions);
...
void MenuWidget::showLeaderboard() {
    // Create a connection to the database
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(QCoreApplication::applicationDirPath() + "/leaderboard.db");
    ...
    // Check if "Steve" already exists in the "user" table
    query.exec("SELECT nickname FROM user WHERE nickname = 'Steve'");
    if (!query.next()) {
        // "Steve" does not exist in the "user" table, so insert "Steve"
        query.prepare("INSERT INTO user (nickname, registrationDate, gamesPlayed, bestRank) "
                      "VALUES (:nickname, :registrationDate, :gamesPlayed, :bestRank)");
        query.bindValue(":nickname", "Steve");
        query.bindValue(":registrationDate", "2023-06-14"); // Set this to the current date
        query.bindValue(":gamesPlayed", 10);
        query.bindValue(":bestRank", 1);
        if (!query.exec()) {
            qDebug() << "Error: Unable to insert data into the user table.";
        }
    }

    // Check if the "score" table already exists
    if (!db.tables().contains("score")) {
        // If not, create the "score" table with a foreign key constraint
        query.exec("CREATE TABLE score ("
                   "playerName TEXT REFERENCES user(nickname),"
                   "levelName TEXT,"
                   "coinsCollected INTEGER,"
                   "timeTaken INTEGER)");
    }

    // Check if "Steve" already exists in the "score" table
    query.exec("SELECT playerName FROM score WHERE playerName = 'Steve'");
    if (!query.next()) {
        // "Steve" does not exist in the "score" table, so insert "Steve"
        query.prepare("INSERT INTO score (playerName, levelName, coinsCollected, timeTaken) "
                      "VALUES (:playerName, :levelName, :coinsCollected, :timeTaken)");
        query.bindValue(":playerName", "Steve");
        query.bindValue(":levelName", "Level1");
        query.bindValue(":coinsCollected", 10);
        query.bindValue(":timeTaken", 120);
        if (!query.exec()) {
            qDebug() << "Error: Unable to insert data into the score table.";
        }
    }

    // Create a model to display the data
    model = new QSqlTableModel(this, db);
    model->setTable("score");
    model->select();

    // Create a view to display the model
    aView = new QTableView;
    aView->setModel(model);
    aView->setSelectionBehavior(QAbstractItemView::SelectRows);
    aView->show();

    aView->setFixedSize(800, 600);
}


void MenuWidget::showPlayerOptions() {
    // The code to connect to the database, open it, create the "user" table, and manage "Steve" is the same as in showLeaderboard.
    ...
    if (!query.next()) {
        query.prepare("INSERT INTO user (nickname, registrationDate, gamesPlayed, bestRank) "
                      "VALUES (:nickname, :registrationDate, :gamesPlayed, :bestRank)");
        query.bindValue(":nickname", "Steve");
        query.bindValue(":registrationDate", "2023-06-14");
        query.bindValue(":gamesPlayed", 10);
        query.bindValue(":bestRank", 1);
        if (!query.exec()) {
            qDebug() << "Error: Unable to insert data into the user table.";
    ...
    aView = new QTableView;
    aView->setModel(model);
    aView->setSelectionMode(QAbstractItemView::SingleSelection);


    // Add an "Add Player" button to the view.
    addPlayerButton = new QPushButton("Add Player", this);
    addPlayerButton->setFixedSize(100, 50);  // Set the size before moving the button.
    addPlayerButton->move(aView->width() - addPlayerButton->width(), 0); // Move the button to the right side.
    connect(addPlayerButton, &QPushButton::clicked, this, &MenuWidget::addUser);

    // Add a "Delete Player" button to the view.
    deletePlayerButton = new QPushButton("Delete Player", this);
    deletePlayerButton->setFixedSize(100, 50);  // Set the size before moving the button.
    deletePlayerButton->move(aView->width() - deletePlayerButton->width(), addPlayerButton->height());  // Move the button to the right side below the addPlayerButton.
    connect(deletePlayerButton, &QPushButton::clicked, this, &MenuWidget::deleteUser);
    ...
    addPlayerButton->setStyleSheet(buttonStyle);
    deletePlayerButton->setStyleSheet(buttonStyle);
    addPlayerButton->setFixedSize(100, 50);
    deletePlayerButton->setFixedSize(100, 50);


    // Add the buttons to the view layout.
    layout = new QVBoxLayout(aView);
...
void MenuWidget::addUser() {
    bool ok;
    QString text = QInputDialog::getText(this, tr("Add Player"),
    ...
    if (ok && !text.isEmpty()) {
        ...
        query.prepare("INSERT INTO user (nickname, registrationDate, gamesPlayed, bestRank) "
                      "VALUES (:nickname, :registrationDate, :gamesPlayed, :bestRank)");
        query.bindValue(":nickname", text);
        query.bindValue(":registrationDate", QDate::currentDate().toString("yyyy-MM-dd"));
        query.bindValue(":gamesPlayed", 0);
        query.bindValue(":bestRank", 0);
        if (!query.exec()) {
            qDebug() << "Error: Unable to insert data into the user table.";
        }
    }
...
void MenuWidget::deleteUser() {
    // Get the current selection.
    QItemSelectionModel *select = aView->selectionModel();
    ...
    if (select->hasSelection()) {
        ...
        if (!selectedIndexes.isEmpty()) {
            // Get the nickname of the selected user.
            QString nickname = selectedIndexes.first().data().toString();

            // Delete the selected user from the "user" table.
            QSqlQuery query;
            query.prepare("DELETE FROM user WHERE nickname = :nickname");
            query.bindValue(":nickname", nickname);
            if (!query.exec()) {
                qDebug() << "Error: Unable to delete user from the user table.";
            }
